...
export function responseText(entry: HarEntry): string | null {
  ...
  if (!text) return null;
  return entry.response.content.encoding === "base64" ? Buffer.from(text, "base64").toString("utf8") : text;
}

export function requestJson(entry: HarEntry): Record<string, unknown> | null {
